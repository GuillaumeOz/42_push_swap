...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   push_swap_load_rb_rra_way.c                        :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: gozsertt <gozsertt@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/07/16 11:11:41 by gozsertt          #+#    #+#             */
/*   Updated: 2021/07/20 20:07:01 by gozsertt         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	change_save_cmd(t_swap_calc *swap_calc, char *result_cmd)
{
	...
	if (swap_calc->tmp_rb_rra_cmd != NULL)
		save_cmd_len = ft_strlen(swap_calc->tmp_rb_rra_cmd);
	else
		save_cmd_len = 0;
	if (save_cmd_len == 0 && result_cmd != NULL)
		swap_calc->tmp_rb_rra_cmd = ft_strdup(result_cmd);
...
static void	calc_rb_rra_right_pos(t_swap_calc *swap_calc, t_stack *tmp_a,
...
static void	load_rb_rra_count(t_swap_calc *swap_calc, t_stack **a_stack,
	t_stack **b_stack, int count)
{
	...
	if (swap_calc->tmp_rb_rra_cmd == NULL)
		...
		< ((int)ft_count_elems(swap_calc->tmp_rb_rra_cmd, '\n')))
		size_max = (get_swap_calc_b_stack_size(swap_calc) - 1);
	else
		size_max = (ft_count_elems(swap_calc->tmp_rb_rra_cmd, '\n'));
	if (count > size_max)
		return ;
	tmp_count = count;
	...
	calc_rb_rra_right_pos(swap_calc, tmp_a, tmp_b, count);
	load_rb_rra_count(swap_calc, a_stack, b_stack, count + 1);
}
	...
	t_stack **b_stack)
{
	...
	load_rb_rra_count(swap_calc, a_stack, b_stack, 0);
}
